...
//                .build();
//
//        //fragment는 getContext, Activity는 this
//        googleApiClient = new GoogleApiClient.Builder(this)
//                .enableAutoManage(this, this)
//                .addApi(Auth.GOOGLE_SIGN_IN_API, googleSignInOptions)
//                .build();
...
//            GoogleSignInResult result = Auth.GoogleSignInApi.getSignInResultFromIntent(data);
//            if(result.isSuccess()){ //인증결과가 성공적이면
//                GoogleSignInAccount account = result.getSignInAccount(); // account라는 데이터는 구글 로그인 정보를 담고 있음 (닉네임, 프로필 사진, email 주소)
...
//        AuthCredential credential = GoogleAuthProvider.getCredential(account.getIdToken(), null); //현재 아이디에 대한 토큰 값을 가져와라
//        auth.signInWithCredential(credential)
//                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
//                    @Override
//                    public void onComplete(@NonNull Task<AuthResult> task) {
...
//
//                            Intent intent = new Intent(getApplicationContext(), ResultActivity.class);
//                            intent.putExtra("nickName", account.getDisplayName()); //구글에서 지정한 자기의 이름
//                            intent.putExtra("photoUrl", String.valueOf(account.getPhotoUrl())); //String.valueof() 특정 자료형을 String 형태로 변환
//
//                            startActivity(intent);
//
